...
class Polls(commands.Cog):
    ...
    async def simplepoll(self, ctx, *question):
        ...
        poll.set_author(name=ctx.author.name, icon_url=ctx.author.avatar_url)
        msg = await ctx.send(embed=poll)
        await msg.add_reaction(self.bot.get_emoji(875760022674636820))
        await msg.add_reaction(self.bot.get_emoji(875760022741733386))
        await ctx.message.delete()
    ...
    async def question(self, ctx, msg_id: int, new_question):
        ...
        if poll_dict.get('title'):
            ...
            poll_dict['description'] = f'```{new_question}```'
